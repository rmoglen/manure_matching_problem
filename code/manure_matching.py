...
def plot_net(G, fig_dir):
	...
	# except:
	# 	pass
	plt.title(' Network', fontsize=15, **csfont)
...
def plot_shipment(model, G, fig_dir, nutrient):
	"""
	args: 
	...
	plt.title('Optimal Manure Matching', fontsize=15, **csfont)
...
def visualize_inputs(G, fig_dir, nutrient):
	...
	plt.title(nutrient+" Balance", **csfont)
	plt.xlabel("Nutrient Balance (lbs)", **csfont)
	plt.ylabel("Number of Counties", **csfont)
	...
	plt.text(30000, 160, f'Mean: {statistics.mean(nx.get_node_attributes(G,nutrient+" balance").values()):.2f} lbs', **csfont)
	plt.text(30000, 150, f'Std Dev: {statistics.stdev(nx.get_node_attributes(G,nutrient+" balance").values()):.2f} lbs', **csfont)
...
plt.title("Percent of Demand Met by Synthetic Fertilizer", **csfont)
plt.xlabel("Percent Demand (%)", **csfont)
plt.ylabel("Number of Counties", **csfont)
plt.axvline(statistics.mean(synthetic_fertilizer), color='k', linewidth=1)
plt.text(80, 110, f'Mean: {statistics.mean(synthetic_fertilizer):.2f}%', **csfont)
plt.text(80, 105, f'Std Dev: {statistics.stdev(synthetic_fertilizer):.2f}%', **csfont)
...
plt.title("Percent of Manure Supply Disposed", **csfont)
plt.xlabel("Percent Supply (%)", **csfont)
plt.ylabel("Number of Counties", **csfont)
plt.axvline(statistics.mean(manure_disposal), color='k', linewidth=1)
plt.text(80, 85, f'Mean: {statistics.mean(manure_disposal):.2f}%', **csfont)
plt.text(80, 80, f'Std Dev: {statistics.stdev(manure_disposal):.2f}%', **csfont)
...
plt.title("Manure Transportation Distances", **csfont)
plt.xlabel("Transportation Distance (miles)", **csfont)
plt.ylabel("Number of Shipments", **csfont)
...
plt.text(500, 55, f'Mean: {statistics.mean(transport_distances):.2f}', **csfont)
plt.text(500, 51, f'Std Dev: {statistics.stdev(transport_distances):.2f}', **csfont)
plt.savefig(fig_dir+ "transport_dist_hist")
plt.close()

plot_shipment(model, G, fig_dir, nutrient)
...
# 		print(str(i)+" "+str(model.z[i].value))